...
class StreamsParser {
  ...
  static List<dynamic> parseInfoItemListFromMap(info,
    ...
    if ((info as Map).containsKey("error")) {
      // Use debugPrint rather than print to satisfy lint rules
      // and avoid accidental production prints.
      debugPrint(info["error"]?.toString());
      return [];
    }
    List<StreamInfoItem> listVideos =
        StreamsParser.parseStreamListFromMap(info['streams']);
    ...
    info['playlists'].forEach((_, map) {
      listPlaylists.add(PlaylistInfoItem(
          ...
          map['name'],
          map['uploaderName'],
          map.containsKey('thumbnails')
              ? List<String>.from(jsonDecode(map['thumbnails']))
              : [],
  ...
  static List<StreamInfoItem> parseStreamListFromMap(info) {
    ...
    info.forEach((_, map) {
      streams.add(StreamInfoItem(
          ...
          map['uploaderName'],
          map['uploaderUrl'],
          map.containsKey('uploaderAvatars')
  ...
  static List<StreamSegment> parseStreamSegmentListFromMap(info) {
    ...
    info.forEach((_, map) {
      segments.add(StreamSegment(map['url'], map['title'], map['previewUrl'],
