...
import 'package:social_flow_data_extracter/models/videoInfo.dart';
...
class VideoExtractor {
  ...
  static Future<YoutubeVideo> getStream(String? videoUrl) async {
    ...
    info[1].forEach((key, map) {
      audioOnlyStreams.add(AudioOnlyStream(
        map['torrentUrl'],
        map['url'],
    ...
    return YoutubeVideo(
        videoInfo: VideoInfo(
          ...
          uploaderName: infoMap.containsKey('uploaderName')
              ? infoMap['uploaderName']
              : null,
          uploaderAvatars: infoMap.containsKey('uploaderAvatars')
              ? List<String>.from(jsonDecode(infoMap['uploaderAvatars']))
              : null,
          uploaderUrl: infoMap.containsKey('uploaderUrl')
              ? infoMap['uploaderUrl']
              : null,
          uploadDate:
              infoMap.containsKey('uploadDate') ? infoMap['uploadDate'] : null,
          description: infoMap.containsKey('description')
              ? infoMap['description']
              : null,
          length: infoMap.containsKey('length')
              ? int.parse(infoMap['length'])
              : null,
          viewCount: infoMap.containsKey('viewCount')
              ? int.parse(infoMap['viewCount'])
              : null,
          likeCount: infoMap.containsKey('likeCount')
              ? int.parse(infoMap['likeCount'])
              : null,
          dislikeCount: infoMap.containsKey('dislikeCount')
              ? int.parse(infoMap['dislikeCount'])
              : null,
          category:
              infoMap.containsKey('category') ? infoMap['category'] : null,
          ageLimit: infoMap.containsKey('ageLimit')
              ? int.parse(infoMap['ageLimit'])
              : null,
          thumbnails: infoMap.containsKey('thumbnails')
              ? List<String>.from(jsonDecode(infoMap['thumbnails']))
              : null,
        ),
  ...
  static Future<List<dynamic>> getRelatedStreams(String videoUrl) async {
    ...
    Future<dynamic> task() => NewPipeExtractorDart.extractorChannel
        .invokeMethod("getRelatedStreams", {"videoUrl": videoUrl});
    var info = await task();
    // Check if we got reCaptcha needed response
    info = await ReCaptchaPage.checkInfo(info, task);
